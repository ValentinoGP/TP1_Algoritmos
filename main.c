...
int main(int argc, char *argv[]) {
    ...
    while (!done) {
        ...
        if (resto_timer > 0 && modelo_resto1) {
            ...
            float px = 5.0f * t;
            float pz = 10.0f * t - 0.5f * 9.81f * t * t;
            if (pz < 0) pz = 0;
            modelo_t *piezas[6] = {modelo_torreta, modelo_radar, modelo_resto1, modelo_resto2, modelo_resto1, modelo_resto2};
            for (int k = 0; k < 6; k++) {
                if (!piezas[k]) continue;
                float angulo = k * (2 * M_PI / 6);
                float wx = resto_x + px * cosf(angulo);
                float wy = resto_y + px * sinf(angulo);
                float sr = (rand() % 628) / 1000.0f;
                float sy = (rand() % 628) / 1000.0f;
                float vec[3] = {wx, wy, pz};
                matriz_t *mt = matriz_crear_mt(vec);
                matriz_t *mrz = matriz_crear_mz(angulo + sr);
                matriz_t *mry = matriz_crear_my(sy);
                matriz_t *obj1 = matriz_multiplicar(mt, mrz);
        ...
        
        {
            ...
            matriz_destruir(hud_cam); hud_cam = tmp;
            matriz_destruir(mper);
            ...
            if (game_over) {
                ...
                for (int i = 0; i < len; i++) {
                    if (msg[i] >= 'A' && msg[i] <= 'Z' && modelo_letras[msg[i] - 'A'])
                        renderizar_modelo(renderer, stack, modelo_letras[msg[i] - 'A'], start_x + i * 0.7f, 0.8f, 0, 0, 0);
                }
                SDL_SetRenderDrawColor(renderer, 0xFF, 0xFF, 0xFF, 0x00);
                int temp = puntaje;
                char buf[10];
                int n = 0;
                if (temp == 0) { buf[n++] = '0'; }
                else { while (temp > 0) { buf[n++] = '0' + temp % 10; temp /= 10; } }
                start_x = -n * 0.9f / 2;
                for (int i = n - 1; i >= 0; i--) {
                ...
                if (hit_timer > 0 && modelo_hash) {
                    SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0x00, 0x00);
                    { size_t hash_lines = modelo_nlineas(modelo_hash); size_t show = (size_t)((1.0f - hit_timer) * hash_lines); if (show > hash_lines) show = hash_lines; renderizar_modelo(renderer, stack, modelo_hash, 0, 0, 0, 0, show); }
                }
                ...
                if (fabs(p_diff) > 1.0f && enemigo) {
                    ...
                    for (int i = 0; i < len; i++) {
                        if (msg[i] >= 'A' && msg[i] <= 'Z' && modelo_letras[msg[i] - 'A'])
                            renderizar_modelo(renderer, stack, modelo_letras[msg[i] - 'A'], start_x + i * 0.7f, 0.5f, 0, 0, 0);
                    }
                }
